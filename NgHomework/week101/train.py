...
# 首先第一步就是加载数据集，包括训练集，测试集，类别等
train_set_x_orig, train_set_y, test_set_x_orig, test_set_y, classes = load_dataset()

# # 打印一张图片看看
# index = 25
# plt.imshow(train_set_x_orig[index])
# plt.show() # 注意这一句要加上去要不然不显示图片
...
num_px = train_set_x_orig.shape[1] # 训练集测试集中图片的宽度和高度（均为64x64）
...
def propagate(w, b, X, Y):
    ...
    # 反向传播,dw和db的表达式，可以通过求偏导自己推出来，就是这样表示的
...
def predict(w, b, X):
    ...
    for i in range(A.shape[1]):
        Y_pridiction[0,i] = 1 if A[0, i] > 0.5 else 0
