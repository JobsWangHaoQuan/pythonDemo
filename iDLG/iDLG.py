# iDLG: Improved Deep Leakage from Gradients的代码复现，学习一下
...
class Dataset_from_Image(Dataset):
    def __int__(self, imgs, labs, transform=None):
        self,imgs = imgs
        self.labs = labs
...
def main():
    ...
    for idx_net in range(num_exp):
        ...
        for method in ['DLG', 'iDLG']:
            print('%s, Try to generate %d image '%(method, num_dummy))
            ...
            for iters in range(Iteration):
                def closure():
                    ...
                    if method == 'DLG':
                        ...
                        dummmy_loss = criterion(pred, label_pred)
                ...
                if iters % int(Iteration /30) == 0:
                    ...
                    for imidx in range(num_dummy):
                        ...
                        for i in range(min(len(history), 29)):
                            ...
                            plt.axis('off')
                        if method == 'DLG':
                            plt.savefig('%s/DLG_on%s_%05d.png'%(save_path, imidx_list, imidx_list[imidx]))
                            plt.close()
                        elif method == 'iDLG':
                            plt.savefig('%s/iDLG_on%s_%05d.png'%(save_path, imidx_list, imidx_list[imidx]))
                            plt.close()
